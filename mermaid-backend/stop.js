...
async function stopBackend() {
  ...
  try {
    // Verificar processos na porta 8096 (Windows)
    const { stdout, stderr } = await execPromise('netstat -ano | findstr :8096');
    ...
    console.log(`📋 Encontrados ${pids.size} processo(s) na porta 8096:`);
    ...
    try {
      const { stdout: checkStdout } = await execPromise('netstat -ano | findstr :8096');
